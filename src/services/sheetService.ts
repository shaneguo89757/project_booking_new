...
export class SheetService {
  ...
  private async fetchSheetData(spreadsheetId: string, range: string) {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to fetch sheet data: ${response.status} ${errorData.error?.message || 'Unknown error'}`
      );
    }
  ...
  private async getSheetId(spreadsheetId: string, sheetName: string): Promise<number> {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to fetch spreadsheet info: ${response.status} ${response.statusText}`);
      }
      ...
      if (!sheet) {
        console.error('Available sheets:', data.sheets?.map((s: any) => s.properties.title));
  ...
  async deleteClassDay(spreadsheetId: string, targetDate: string): Promise<boolean> {
    try {
      if (!this.accessToken) {
      ...
      // 1. 獲取工作表 ID
      const sheetId = await this.getSheetId(spreadsheetId, this.SHEETS.CLASS_DAYS);
      ...
      if (!deleteResponse.ok) {
        throw new Error(`Failed to delete row: ${deleteResponseData.error?.message || 'Unknown error'}`);
      }
  ...
  async addClassDay(spreadsheetId: string, date: string): Promise<boolean> {
    try {
      ...
      const dateExists = existingDates.some(row => 
        normalizeDate(row[0]) === normalizeDate(formattedDate)
      );
